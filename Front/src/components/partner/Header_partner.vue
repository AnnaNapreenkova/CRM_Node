<template>
  <div
    class="wrap partner-header ducking-header"
  ...
  >
    ...
    <ul class="left-menu-crap">
      <router-link class="home-link" exact :to="'/partner'"
      ...
      >
      <router-link class="project-link" exact v-show="functionality != 'none'" :to="'/services'">
      ...
      >
       ...
       <router-link class="clients-link" exact v-show="functionality != 'none'" :to="'/partner/clients'">
        <li class="clients-tab clever-animation">
          <img src="@/assets/icons/project-page-icon.svg" alt="" />
          <p class="mobile-hide clever-animation">Клиенты</p>
          <div class="notification-bubble mobile-hide clever-animation">
            <p>{{ clients.length }}</p>
          </div>
        </li></router-link
      ...
      <router-link class="kanban-link" exact v-show="functionality != 'none'" :to="'#'">
      ...
      >
      <router-link class="analytics-link" exact v-show="functionality != 'none'" :to="'##'">
      ...
      >
      <router-link class="calendar-link" exact v-show="functionality != 'none'" :to="'###'"
      ...
      >
      <router-link class="messages-link" exact v-show="functionality != 'none'" :to="'####'">
      ...
      > -->
      <router-link v-show="functionality != 'none'" :to="'/'" v-if="isClientInfo">
        <button class="exit-tab clever-animation">
          <img src="@/assets/icons/exit-logout.svg" style="width: 25px: margin-right: 5px;" alt="" />
          <p class="m-0 mobile-hide clever-animation">Выход</p>
        </button></router-link
      >
    </ul>
     <b-button variant="123" @click="logOut()" class="offer-try header-exit"
                  > <img class="img-fluid clever-animation"
        style="width: 24px; filter: brightness(0);" src="@/assets/icons/exit-logout.svg" alt="" />
          <p class="m-0 mobile-hide clever-animation">Выход</p></b-button>
    <div class="copyright clever-animation">
...
export default {
  ...
  computed: {
    ...PartnerMapper.mapState(["partnerProfile", "clients"]),
  },
  directives: {
    clickOutside: vClickOutside.directive,
  },
  methods: {
    ...PartnerMapper.mapActions(["getPartnerProfile", "getClients", "getPartnerServices"]),
  ...
  async mounted() {
    ...
    let services = await this.getPartnerServices();
    if (services) {
...
.left-menu-crap > a > .clients-tab:hover,
.clients-link.router-link-active > .clients-tab {
  background-color: rgb(137 9 145);
}

.clients-tab > .notification-bubble {
  background-color: rgb(137 9 145);
}
