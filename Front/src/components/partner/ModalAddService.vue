<template>
  ...
  >
    ...
    <div class="w-100 m-0 modal-body pl-3 h-auto pb-0">
      <h2 class="client-header-custom">Новая услуга</h2>
...
export default {
  ...
  methods: {
    chooseItem(item, index) {
      this.$refs.serviceInput.setAttribute("placeholder", item);
      this.$refs.serviceInput.setAttribute("value", item);
      this.$refs.serviceInput.classList.add("input-color");
      this.isShownList = false;
      this.$refs.inputOpened.classList.remove("opened");
      this.checkedService = index;
    },

    showList() {
      this.isShownList = !this.isShownList;
      this.$refs.inputOpened.classList.toggle("opened");
    },
  },
  computed: {
    ...
    service() {
      ...
      if (this.checkedService == 1 || this.checkedService == 3) {
        end_data = null;
        days = null;
      } else {
        end_data = moment().add(1, "M");
        let a = moment(end_data);
        let b = moment();
      ...
      if (
        ...
        this.partnerProfile.functionality &&
        this.partnerProfile.functionality == "client"
      ) {
      ...
      return {
        client: client,
        service_type: this.checkedService,
        service_price: this.newService.price,
        service_status: "not_purchased",
        end_data: end_data,
        days: days,
        agreement: this.newService.agreement.split(' ')[1],
      };
    },
  },
};
