<template>
  ...
  >
    ...
    <div class="w-100 m-0 modal-body pl-3 h-auto pb-0">
      <h2 v-if="agreementType == 'agreement'" class="client-header-custom">
        Новый договор
      </h2>
      <h2
        v-else-if="agreementType == 'ext_agreement'"
        class="client-header-custom"
      >
      ...
      <form class="client-form">
        <div class="row client-inputs client-inputs-services mt-3">
          ...
          >
            ...
            <input
              ref="clientInput"
              v-model="newAgreement.service"
              placeholder="Сервисы клиента"
              type="text"
        ...
        <div class="row client-inputs mt-3">
          <input
            ref="agreementName"
            v-model="newAgreement.doc_name"
            placeholder="Называние договора"
            type="text"
            class="form-control pl-3"
          />
        </div>
        <div class="row client-inputs mt-3">
          <input
            v-model="newAgreement.doc_number"
        ...
        <div
          v-if="agreementType == 'ext_agreement'"
          class="row client-inputs mt-3"
        >
          <input
            v-model="newAgreement.extdoc_number"
            placeholder="Номер дополнительного соглашение"
            type="text"
            v-mask="'№: XXXXXXXXXXX'"
            class="form-control pl-3"
          />
        ...
        >
          <input
            v-model="serviceDate"
            placeholder="Дата окончание услуги"
            type="date"
            class="form-control pl-3"
          />
        </div>
...
export default {
  ...
  methods: {
    chooseClient(item) {
      this.$refs.clientInput.setAttribute(
        "placeholder",
        this.serviceList[item.service_type]
      );
      this.newAgreement.doc_name = `Договор на услугу ${
        this.serviceList[item.service_type]
      }`;
      // this.$refs.clientInput.setAttribute("value", this.serviceList[item.service_type]);
      this.$refs.clientInput.classList.add("input-color");
      this.$refs.agreementName.setAttribute(
        "placeholder",
        `Договор на услугу ${this.serviceList[item.service_type]}`
      );
  ...
  computed: {
    ...
    servicename() {
      let name;
      if (this.agreementType == "ext_agreement") {
        name = this.newAgreement.service =
          this.serviceList[
            this.services.filter(
              (service) => service.id == this.currentAgreement.services
            ).service_type
          ];
      } else {
        name = "";
    ...
    agreement() {
      ...
      // let service = this.services.filter(service => service.id == this.currentAgreement.services);
      ...
      if (this.agreementType == "ext_agreement") {
        headAgreement = this.currentAgreement.id;
        doc_number = this.newAgreement.extdoc_number.split(' ')[1];
        services = this.currentAgreement.services;
        doc_status = this.currentAgreement.doc_status;
      } else {
        headAgreement = null;
        doc_number = this.newAgreement.doc_number.split(' ')[1];
      ...
      return {
        ...
        doc_name: this.newAgreement.doc_name,
        doc_type: this.agreementType,
        head_agreement: headAgreement,
        // client: this.$route.params.id,
        client: this.currentAgreement.client,
  ...
  beforeUpdate() {    
    ...
    if (this.agreementType == "ext_agreement") {
      this.newAgreement.service =
        this.serviceList[
          this.services.filter(
            (service) => service.id == this.currentAgreement.services
          )[0].service_type
        ];
      this.newAgreement.doc_name = `Дополнительное соглашение для сервиса ${this.newAgreement.service}`;
      this.newAgreement.doc_number = "№ " + this.currentAgreement.doc_number;
    }
  },
};
</script>
